...
async function initDatabase() {
  ...
  if (!checkEnvVariables()) {
    console.warn('⚠️  Server will start but database operations will fail until .env is configured');
    return;
  }
  ...
  try {
    ...
    console.error('   3. Ensure database exists: CREATE DATABASE ' + process.env.DB_NAME);
...
function checkDbConnection(req, res, next) {
  if (!dbConnected || !db) {
    return res.status(503).json({ 
      success: false, 
      error: 'Database not connected. Please check your .env configuration and ensure MySQL is running.' 
    });
  }
  next();
}
...
app.put('/api/items/:id', checkDbConnection, async (req, res) => {
  try {
    ...
    await db.execute(
      ...
      [name || existing[0].name, description !== undefined ? description : existing[0].description, status || existing[0].status, req.params.id]
    );
...
app.get('/api/health', async (req, res) => {
  if (!dbConnected || !db) {
    return res.status(503).json({ 
      ...
      details: 'Please check your .env configuration and ensure MySQL is running'
    });
  }
...
app.listen(PORT, () => {
  ...
  console.log('');
  if (!dbConnected) {
    console.log('⚠️  Note: Database is not connected. Please configure .env file.');
  }
  console.log('');
});
